...
use crate::block::{CompactBlockChunks, PoolFilter, StoredBlock, StoredBlockChunks};
...
pub trait StoredBlockRead: ChainStoreReader {
    ...
    ) -> impl Future<Output = Result<StoredBlockChunks, ChainStoreError>> + Send;
}
...
pub trait CompactBlockRead: ChainStoreReader {
    ...
    fn compact_stream(
        ...
        pools: PoolFilter,
    ) -> impl Future<Output = Result<CompactBlockChunks, ChainStoreError>> + Send;
}
