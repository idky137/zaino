...
use zaino_primitives::types::{
    BlockHeader, BlockRef, ChainWork, CompactBlock, PreIndexCompactTx, ShieldedPool,
    SignedZatoshis, TreeRoots,
};
...
/// A stream of block chunks.
///
/// Chunked rather than per-block because a store reads a range by walking a
/// cursor under one transaction, and a chunk is the natural unit of that walk:
/// one transaction opened, one batch handed over. Streaming single blocks
/// would put a channel send between every block on the hot path, for no gain —
/// the reader is going to consume them in order regardless.
///
/// A named type rather than an anonymous `impl Stream` for the reasons
/// `zaino_chain_head::ChainHeadBlockIter` is named: a consumer can name what
/// it holds, every implementation yields the same thing, and the port stays
/// object-safe.
///
/// Chunk boundaries carry no meaning. They are where the store chose to end a
/// transaction, and a consumer must not read anything into them.
pub struct BlockChunks<T> {
    inner: Pin<Box<dyn Stream<Item = Result<Vec<T>, ChainStoreError>> + Send>>,
}

impl<T> BlockChunks<T> {
    /// Wraps a stream of chunks.
    pub fn new(
        inner: impl Stream<Item = Result<Vec<T>, ChainStoreError>> + Send + 'static,
    ) -> Self {
        Self {
            inner: Box::pin(inner),
        }
...
impl<T> Stream for BlockChunks<T> {
    type Item = Result<Vec<T>, ChainStoreError>;
...
impl<T> core::fmt::Debug for BlockChunks<T> {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.debug_struct("BlockChunks").finish_non_exhaustive()
    }
}

/// Chunks of [`StoredBlock`].
pub type StoredBlockChunks = BlockChunks<StoredBlock>;

/// Chunks of [`CompactBlock`].
pub type CompactBlockChunks = BlockChunks<CompactBlock>;
