...
use zaino_chain_store::{
    ...
    ChainStoreSource, ChainStoreSourceError, CompactBlockChunks, CompactBlockRead, MigrationState,
    PoolFilter, SchemaVersion, SpenderRef, SpentOutputIndex, StoreCapabilities, StoreCapability,
    StoreSchema, StoreWatermark, StoredAddress, StoredBlock, StoredBlockChunks, StoredBlockRead,
    StoredTx, StoredTxOut, TransactionIndex, TxOutSetAccumulator, TxOutSetIndex,
};
...
/// act on.
fn store_capabilities(capability: Capability) -> StoreCapabilities {
    let mut capabilities = vec![StoreCapability::Core];
    ...
    if capability.has(
        ...
        capabilities.push(StoreCapability::StoredBlocks);
    }
    if capability.has(Capability::COMPACT_BLOCK_EXT) {
        capabilities.push(StoreCapability::CompactBlocks);
    }
    if capability.has(Capability::BLOCK_CORE_EXT) {
        capabilities.push(StoreCapability::Transactions);
    }
    ...
    if capability.has(Capability::SPENT_OUTPUT_INDEX.union(Capability::BLOCK_TRANSPARENT_EXT)) {
        capabilities.push(StoreCapability::SpentOutputs);
    }
    if capability.has(Capability::TXOUT_SET_INDEX) {
        capabilities.push(StoreCapability::TxOutSet);
    }
...
impl<T: ChainStoreSource> ChainStoreReader for DbReader<T> {
    ...
    fn capabilities(&self) -> StoreCapabilities {
        store_capabilities(self.inner.capability())
    }
    ...
    Fut: Future<Output = Result<Vec<B>, ChainStoreError>> + Send + 'static,
{
    ...
    futures::stream::try_unfold((Some(start), read), move |(cursor, mut read)| async move {
...
impl<T: ChainStoreSource> StoredBlockRead for DbReader<T> {
    ...
    ) -> Result<StoredBlockChunks, ChainStoreError> {
        let reader = self.clone();
        let Some((start, end)) = self.clamped_range(start, end)? else {
            return Ok(StoredBlockChunks::new(futures::stream::empty()));
        };

        Ok(StoredBlockChunks::new(chunked(
            start,
            end,
            move |from, to| {
...
impl<T: ChainStoreSource> CompactBlockRead for DbReader<T> {
    ...
    async fn compact_stream(
        ...
        pools: PoolFilter,
    ) -> Result<CompactBlockChunks, ChainStoreError> {
        let reader = self.clone();
        let Some((start, end)) = self.clamped_range(start, end)? else {
            return Ok(CompactBlockChunks::new(futures::stream::empty()));
        };

        Ok(CompactBlockChunks::new(chunked(
            start,
            end,
            move |from, to| {
                let reader = reader.clone();
...
mod tests {
    ...
    fn a_store_missing_transparent_rows_does_not_claim_spent_outputs() {
        ...
        assert!(!store_capabilities(without_rows).contains(StoreCapability::SpentOutputs));

        let with_rows = Capability::SPENT_OUTPUT_INDEX | Capability::BLOCK_TRANSPARENT_EXT;
        assert!(store_capabilities(with_rows).contains(StoreCapability::SpentOutputs));
    }
    ...
    fn core_is_always_claimed() {
        assert!(store_capabilities(Capability::empty()).contains(StoreCapability::Core));
    }
